const skills = [
  ...
  { name: 'Machine Learning (scikit-learn, XGBoost)', color: 'bg-blue-600' },
  { name: 'Deep Learning (PyTorch / TensorFlow)', color: 'bg-yellow-400' },
  { name: 'NLP & LLMs (RAG, LangChain, OpenAI API)', color: 'bg-orange-400' },
  { name: 'SQL & Data Modeling', color: 'bg-cyan-400' },
  { name: 'Data Engineering (ETL,Airflow, Spark)', color: 'bg-purple-400' },
  { name: 'Cloud (AWS/Azure/GCP)', color: 'bg-pink-400' },
  { name: 'MLOps (Docker, CI/CD)', color: 'bg-green-400' },
];


export function Skills() {
  return (
    <section id="skills" className="min-h-screen py-20 bg-neon-green">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {skills.map((skill, index) => (
            ...
            >
              ...
              {/* Card */}
              <div className={`relative ${skill.color} border-4 border-black p-6 transition-transform group-hover:-translate-y-1 h-32 flex items-center justify-center`}>
                <h3 className="font-jetbrains font-extrabold text-xl text-center leading-tight">
                  {skill.name}
                </h3>
              </div>
            </div>
        ...
        {/* About Section */}
        <div className="mt-16 bg-white border-4 border-black p-8 md:p-12 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
          <h3 className="font-jetbrains font-extrabold text-3xl sm:text-4xl mb-6">
            About Me
          </h3>
          <div className="font-jua text-lg sm:text-xl space-y-4 max-w-3xl">
          <p>
